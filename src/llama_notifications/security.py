...
class EncryptionService:
    ...
    def _crypto_encrypt(
        self, data: str, key: str, encryption_type: EncryptionType
    ) -> str:
        """
    ...
    def _simulate_encryption(
        ...
        if encryption_type == EncryptionType.NONE:
            ...
            aes_part = base64.b64encode(key_hash1 + data.encode("utf-8")).decode(
                "utf-8"
            )
    ...
    def simulate_tee_protected_processing(
        self, data: str, operation: str
    ) -> Dict[str, Any]:
        """
...
class TokenManager:
    ...
    def generate_token(self, user_id: str, service: str, expiry: int = 86400) -> str:
        ...
        master_key = os.environ.get(
            "LLAMA_NOTIFICATIONS_MASTER_KEY", "default-master-key"
        )
        encrypted = self.encryption_service.encrypt(
            payload_json, master_key, EncryptionType.AES256
        )
